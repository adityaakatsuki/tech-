...
def _ensure_scraped_record_columns():
    """
    Add any missing columns to the scraped_records table.
    Works with both SQLite and PostgreSQL.
    """
    inspector = inspect(engine)

    # Table doesn't exist yet
    if "scraped_records" not in inspector.get_table_names():
        return

    existing_cols = {
        column["name"]
        for column in inspector.get_columns("scraped_records")
    }
        ...
        for col in ScrapedRecord.__table__.columns:
            if col.name not in existing_cols:
                ...
                conn.exec_driver_sql(
                    f'ALTER TABLE scraped_records ADD COLUMN "{col.name}" {col_type}'
                )
